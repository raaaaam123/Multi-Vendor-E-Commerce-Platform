...
const Categories = () => {
  ...
  const { categories, loading, error, pagination, actionLoading, actionError } =
  ...
  useEffect(() => {
    dispatch(fetchCategories({ search, page, limit: 10 }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await dispatch(
          updateCategory({ id: editing._id, data: form })
        ).unwrap();
      } else {
